...
The current hosted release workflow runs on Apple Silicon and produces `arm64` DMG artifacts. Intel and universal macOS builds are not currently published.

The signed workflow writes `signed-notarized` DMG and manifest artifacts plus a `release-security-evidence-<version>-<arch>.json` report. The report records independent `codesign`, hardened-runtime, `spctl`, stapler, and SHA-256 checks. Before upload, the workflow copies the final files into `release-artifacts/` and runs `pnpm release:bundle:verify` there, writing `HTMLslide-release-bundle-evidence.json`; the verifier resolves only bundle-relative manifest references, so the candidate remains valid after download or relocation. Tag runs upload that exact bundle as an Actions artifact and create a Draft GitHub Release. The separate promotion workflow verifies the candidate and completed checklist before publishing.
...
Every release candidate must also have a completed manual acceptance checklist generated with `pnpm rc:checklist`. The generated metadata binds the checklist to the package manifest's source commit, manifest SHA-256, and primary DMG SHA-256. The Alpha Package workflow uploads a prefilled checklist template next to its artifacts. The signed Release macOS workflow uploads its template into a Draft GitHub Release; it is not a claim that manual acceptance passed until a tester completes it and uploads `HTMLslide-release-rc-acceptance-completed.md`. Promotion verification rechecks those fingerprints against the exact downloaded bundle.
