...
The desktop app starts external agents from Electron main for an opened local deck project. The renderer sends only the selected adapter, project path, and user brief. Electron owns the run registry, writes project-local run inputs, creates the checkpoint, launches the child process, and publishes bounded sanitized progress snapshots.
...
Generic mode keeps its project-local `.htmlslide/runs/<runId>/prompt.md` and `.htmlslide/runs/<runId>/writes.json` contract. HTMLslide validates the reported write manifest for compatibility, while the checkpoint diff remains the user-visible review and revert surface.
...
External agents may edit source areas described by the project-structure spec. Built-in Claude and Codex adapters operate on an isolated copy and apply only checkpoint-derived source writes; Generic commands additionally report writes through their manifest. Any resolved source path that escapes the project, including a symlink escape discovered after the command completes, is a boundary violation.
