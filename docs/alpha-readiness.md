...
| External-agent RC evidence verifier | `pnpm rc:external-agent-evidence` accepts the packaged app's sanitized run report (or the fixed fixture input), requires accumulated successful and cancelled runs with Check, Export, diff review, and revert evidence, binds it to the caller-declared commit and exact package manifest SHA-256, and emits metadata-only evidence. |
...
The generated checklist lives under `dist/acceptance/` and is intentionally not committed. When `--package-manifest` and `--commit` are supplied, it records the exact package manifest and primary DMG fingerprints; promotion verification fails if the checklist is reused with another package. The Alpha Package workflow uploads a prefilled, incomplete checklist alongside its candidate artifacts. The signed Release macOS workflow uploads the same template into a Draft GitHub Release; after testing, upload the completed file as `HTMLslide-release-rc-acceptance-completed.md` and use the separate promotion workflow. Attach or paste the completed evidence into the release candidate notes as a secondary record.
...
For a real Claude Code or Codex CLI compatibility claim, run the packaged app through a successful edit, Check, Export, diff review, revert, and a distinct cancelled run. The app writes the accumulated sanitized report under `.htmlslide/reports/agent-run-<run-id>.json`; pass that report to `pnpm rc:external-agent-evidence` with the exact package manifest and the stable `htmlslide-signed-notarized-<candidate-run-id>-<dmg-file-name>` artifact reference from the checklist. Upload the resulting sanitized file as `HTMLslide-external-agent-acceptance-evidence.json`; promotion rejects fixture-only output and verifies its provider, commit, artifact reference, and package manifest SHA-256 against the candidate.
