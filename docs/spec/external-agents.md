...
The desktop app starts external agents from Electron main for an opened local deck project. The renderer sends only the selected adapter, project path, and user brief. Electron owns the run registry, writes project-local run inputs, creates the checkpoint, copies source into an OS-temporary workspace, launches the child process there, applies only the verified source diff back to the real project, and publishes bounded sanitized progress snapshots.
