...
- PDF verification covers page count, normalized metadata, repeated-byte determinism for the pinned Chromium build, generation from the same staged DOM as thumbnails, and per-page Poppler raster goldens under `packages/compiler/test/goldens/<fixture>/pdf/<platform>-<arch>/`. CI installs and verifies `pdftoppm`; missing executable, failed command, timeout, invalid output, or golden mismatch fails the gate and writes PDF before/after/diff artifacts under `dist/visual-regression/pdf/`. Cross-machine PDF byte equality remains out of scope.
...
- Desktop Electron E2E covers onboarding, project library, New Deck source-file and pasted-text intake, mock generation, checks, QA panel role semantics, export, presenter, deckpkg open, CLI setup, and official skills setup.
...
- Release-candidate acceptance uses `pnpm rc:checklist` to generate the mandatory manual evidence template for clean-account install, first launch, provider flows, fake external agent automation, real Claude/Codex claim validation or explicit no-claim N/A, Gemini detection-only status, export, external monitor presentation, reopen, revert, CLI uninstall, and post-delete cleanup. A completed real-provider desktop run additionally uses `pnpm rc:byok-evidence` to bind sanitized provider validation, an explicit 8-12 slide run report, compatible endpoint hash, checkpoint snapshot integrity, authoritative check/export, current source fingerprints, and artifact hashes. A real Claude/Codex compatibility claim additionally uses `pnpm rc:external-agent-evidence` to validate the fixed sanitized evidence shape and bind it to the candidate commit and package manifest SHA-256. Release-script tests cover all evidence commands plus release-note rendering and fail-closed evidence cases. Alpha Package uploads a prefilled but incomplete checklist next to candidate artifacts; signed tag candidates use the Draft Release and separate promotion workflow so manual evidence stays tied to the exact run.
- Release evidence script tests cover deterministic RC checklist, BYOK evidence verification, external-agent evidence verification, and release-note rendering. The BYOK matrix covers wrong run/provider/count, endpoint mismatch, malformed reports/manifests, stale sources/artifacts, checkpoint tampering, project-source secrets, and symlinked report/output paths. The external-agent matrix covers fixed provider/auth commands, source-path safety, cancellation ordering, secret rejection, and alpha/release package signing contracts.
