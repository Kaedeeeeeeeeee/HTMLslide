...
- `htmlslide rc byok --project <path> --provider openai|anthropic|compatible --model <model> --api-key-env <ENV_NAME> --task <brief> [--target-slide-count 8..12] [--base-url <url>] [--commit <commit>] [--artifact-url <url-or-label>] [--artifact-sha256 <sha256>] --json` validates a real provider, runs the provider-backed agent, forces PDF/deckpkg/thumbnail export, and writes sanitized run-bound evidence under `.htmlslide/reports/rc-evidence-<run-id>/`. It requires the exact 8-12 slide target and a zero-error authoritative Check. For release promotion, pass the candidate DMG SHA-256 from the verified bundle so the evidence is bound to the actual candidate bytes.
- `pnpm rc:byok-evidence -- --project <path> --provider-validation <validation.json> [--run-id <id>] [--report <agent-report.json>] [--output <evidence.json>] [--commit <commit>] [--artifact-url <url-or-label>] [--artifact-sha256 <sha256>]` verifies a completed desktop BYOK run without reading provider credentials.
