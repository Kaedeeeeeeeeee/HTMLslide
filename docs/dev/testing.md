...
- Release evidence script tests: deterministic coverage for `rc:checklist`, `rc:byok-evidence`, `rc:external-agent-evidence`, `release:security:verify`, and `release:notes`, including run-bound metadata, 8-12 slide/provider/report/export consistency, fixed real Claude/Codex evidence shape, package-manifest binding, independent signature/Gatekeeper/staple/hash evidence, secret rejection, manual evidence sections, signed-release gates, and empty release-range warnings.
