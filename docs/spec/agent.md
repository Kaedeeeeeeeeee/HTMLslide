...
For v1, HTMLslide Agent and Coding Agent are visible as product modes with readiness state. API keys are saved through Electron credential storage instead of project/settings JSON, but provider-backed BYOK generation remains blocked until a real provider run path exists. Coding Agent generation is enabled only for a saved Generic command in the opened workspace Generate path; New Deck generation remains blocked for Claude/Codex until headless templates are defined. This prevents the alpha app from silently treating unsupported modes as source-only generation.
