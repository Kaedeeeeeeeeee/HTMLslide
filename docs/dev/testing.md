...
- Release evidence script tests: deterministic coverage for `rc:checklist`, `rc:byok-evidence`, and `release:notes`, including run-bound metadata, 8-12 slide/provider/report/export consistency, secret rejection, manual evidence sections, signed-release gates, and empty release-range warnings.
