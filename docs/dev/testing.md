...
- CLI E2E tests: `htmlslide new`, `htmlslide check --json`, `htmlslide export --pdf --deckpkg`, `htmlslide mcp --list-tools --json`, `htmlslide mcp <project> --status --json`, `htmlslide package`, `htmlslide doctor`.
...
- MCP tests: verify CLI discovery/status, server startup, tool listing, path boundary enforcement, schema-valid reports, and artifact creation.
