...
- Compiler regression tests: golden decks for minimal, text-heavy, data chart, image-heavy, notes, and multi-theme decks, including byte-exact fallback thumbnail PNG baselines for deterministic compiler output. Export transaction coverage must verify one source snapshot, project-lock exclusion and token-claimed stale-lock recovery, staging cleanup after failure, destination preflight, preservation of the previous commit after a rejected partial export, manifest-last atomic replacement, deterministic SHA-256 metadata, symlink boundary rejection, and partial-export removal of only old manifest-owned artifacts.
...
- MCP tests: verify CLI discovery/status, stdio server startup, tool listing, render tools, path boundary enforcement, schema-valid reports, and artifact creation.
- Electron and presenter tests: cover onboarding, workspace choice, project and deckpkg launch arguments, second-instance open forwarding, mock agent deck creation, preview, checks, export, rehearsal mode, settings, notes, next/previous navigation, timer, keyboard shortcuts, and deckpkg resource-limit rejection.
...
- Release evidence script tests: deterministic coverage for `rc:checklist` and `release:notes` rendering, including run-bound metadata, manual evidence sections, signed-release gates, and empty release-range warnings.
