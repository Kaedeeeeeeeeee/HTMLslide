...
The signed workflow writes `signed-notarized` DMG and manifest artifacts plus a `release-security-evidence-<version>-<arch>.json` report. The report records independent `codesign`, hardened-runtime, `spctl`, stapler, and SHA-256 checks. The workflow verifies that report against the final manifest before upload, then attaches the release artifacts to the matching GitHub Release when run from a `v*` tag.
